...
use metasearch::models::{Category, ResultItem, TimeRange};
...
pub async fn search(
    ...
    opts.category = match req.topic.as_deref() {
    ...
    if req.include_raw_content {
        ...
        for r in &mut results {
            match metasearch::extract(client, &r.url, 8000, Some(&req.query)).await {
