...
struct SearchParams {
    ...
    query: String,
    #[schemars(description = "Comma-separated engine names (default: all available for the category). See list_engines.")]
    #[serde(default)]
...
struct SuggestParams {
    ...
    query: String,
    #[schemars(description = "Source: duckduckgo, google, bing, brave, startpage, qwant or wikipedia (default: all)")]
    #[serde(default)]
...
impl MetaSearchMcp {
    ...
    fn build_opts(p: &SearchParams, category: Category) -> SearchOptions {
        ...
        if let Some(es) = &p.engines {
            opts.engines = es.split(',').map(str::trim).filter(|s| !s.is_empty()).map(str::to_string).collect();
        }
...
impl MetaSearchMcp {
    #[tool(description = "Search the web across multiple metasearch engines. Returns ranked results with title, url, description and the engines that found each one.")]
    async fn web_search(&self, Parameters(p): Parameters<SearchParams>) -> String {
        self.run_search(&p, Category::Web).await
    }

    #[tool(description = "Search images across multiple engines. Returns direct image urls, thumbnails and dimensions.")]
    async fn image_search(&self, Parameters(p): Parameters<SearchParams>) -> String {
        self.run_search(&p, Category::Images).await
    }

    #[tool(description = "Search news across multiple engines. Returns articles with published date and source.")]
    async fn news_search(&self, Parameters(p): Parameters<SearchParams>) -> String {
        self.run_search(&p, Category::News).await
    }

    #[tool(description = "Search videos across multiple engines. Returns video url, duration, views and uploader.")]
    ...
    #[tool(description = "Fetch a URL and extract its readable main content. Use for grounding answers on the sources returned by web_search.")]
    async fn fetch_page(&self, Parameters(p): Parameters<FetchParams>) -> String {
        match metasearch::extract(&self.client.http(), &p.url, p.max_chars.unwrap_or(8000), None).await
        {
        ...
        match &p.source {
            Some(name) => {
                let Some(source) = metasearch::SuggestSource::from_name(name) else {
                    return serde_json::json!({"error": format!("unknown source '{name}'")}).to_string();
                };
                match metasearch::suggest(&self.client.http(), source, &p.query, region).await {
                    Ok(list) => serde_json::json!({"query": p.query, "source": name, "suggestions": list}).to_string(),
                    Err(e) => serde_json::json!({"error": e.to_string()}).to_string(),
                }
            }
            None => {
    ...
    #[tool(description = "List the search engines available per category. Pass engine names to other tools to restrict them.")]
    ...
    #[tool(description = "Grounded search for RAG: returns a synthesized answer plus ranked sources with content. Prefer this over web_search + fetch_page for single-shot questions.")]
