...
impl Engine for Qwant {
    ...
    async fn search(&self, ctx: &EngineContext<'_>) -> Result<Vec<RawResult>> {
        ...
        if resp.status().as_u16() == 403 {
            return Err(Error::RateLimited("qwant: access denied".into()));
        }
        let body = resp.bytes().await.map_err(Error::from)?;
        let json: serde_json::Value =
            serde_json::from_slice(&body).map_err(|e| Error::Parse(format!("qwant: {e}")))?;
        if json.get("status").and_then(|s| s.as_str()) != Some("success") {
            let err = json.get("error_code").and_then(|e| e.as_i64()).unwrap_or(0);
            if err == 24 {
                return Err(Error::RateLimited("qwant: rate limited".into()));
            }
            if json.get("url").is_some() {
                return Err(Error::RateLimited("qwant: captcha".into()));
            }
            return Ok(Vec::new());
        }
...
mod tests {
    ...
    fn parse_fixture() {
        ...
        if crate::engines::util::is_block_page(text) {
            return;
        }
